...
sub new
{
	my( $class, $world, $mc_ref_x, $mc_ref_z, $e, $n ) = @_;
...
sub context 
{
  ...
  my($lat, $lon) = Elevation::en_to_ll($e,$n);
	...
	if( $opts{ELEVATION} )
	{	
		my $dsm = $opts{ELEVATION}->ll( "DSM", $lat, $lon );
		my $dtm = $opts{ELEVATION}->ll( "DTM", $lat, $lon );
		$el = $dtm;
	...
	{
		$block = $opts{MAPTILES}->block_at( $lat,$lon );
	}
