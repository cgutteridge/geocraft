...
sub parse_file
{
	...
	return $self->NBT::Parser::parse_string( $data );
}
...
sub typed_tag
{
	...
	if( $type == 0 ) { return bless {}, "NBT::Tag::End"; }
...
sub tag_compound
{
	...
	{
		my $child = $self->tag;
		return $v if( $child->isa( "NBT::Tag::End" ) );
